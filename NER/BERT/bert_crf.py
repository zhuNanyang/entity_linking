...
flags.DEFINE_string(
    "data_dir", "/home/zfsun3/zhunanyang/other/BERT",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", "/home/zfsun3/zhunanyang/other/BERT/chinese_L-12_H-768_A-12/chinese_L-12_H-768_A-12/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "Entity", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "/home/zfsun3/zhunanyang/other/BERT/chinese_L-12_H-768_A-12/chinese_L-12_H-768_A-12/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", "/home/zfsun3/zhunanyang/other/BERT/output",
    "The output directory where the model checkpoints will be written.")

flags.DEFINE_string(
    "init_checkpoint", "/home/zfsun3/zhunanyang/other/BERT/chinese_L-12_H-768_A-12/chinese_L-12_H-768_A-12/bert_model.ckpt",
...
flags.DEFINE_float("num_train_epochs", 15,
...
flags.DEFINE_float("dropout_rate", 0.6, "dropout keep rate")
...
class DataProcessor(object):
    ...
    @classmethod
    def _read_data(cls, input_file, train=True):
        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in LINE:
                ...
                data.append({"text": text, "mention_data": mention})
        DATA = []
        random_order = list(range(len(data)))
        np.random.shuffle(random_order)
        dev_data = [data[j] for i, j in enumerate(random_order) if i % 9 == mode]
        train_data = [data[j] for i, j in enumerate(random_order) if i % 9 != mode]
        if train:
            n = len(train_data)
            idxs = list(range(n))
    ...
    @classmethod
    def _read_test_data(cls, input_file):
        test_data = []
        D = pd.read_csv('H:/conpetition/competition_12/event_type_entity_extract_eval.csv', encoding='utf-8',header=None)
        D = D[D[2] != u'其他']
...
class NerProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "train.json"), train=True), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "train.txt"), train=False), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_test_example(
            self._read_test_data(os.path.join(data_dir, "event_type_entity_extract_eval.csv")), "test")

    def get_labels(self):
        return ["O", "B_LOC", "I_LOC", "E_LOC", "X", "[CLS]", "[SEP]"]
    def _create_test_example(self, lines, set_type):
        examples = []
...
def convert_single_example_test(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    label_map = {}
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    textlist = [t for t in example.text]
...
def create_model(bert_config, is_training, input_ids, input_mask, sequence_lens,
    ...
    blstm_crf = BiLSTM(embedded_chars=embedding, hidden_sizes=FLAGS.hidden_sizes, layers=FLAGS.layers,
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        sequence_lens = tf.reduce_sum(used, 1)
...
def main(_):
    ...
    processors = {"entity": NerProcessor}
    ...
    if FLAGS.do_predict:
        ...
        print("predict_examples:{}".format(predict_examples))
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "label_test_txt")
        def result_to_pair(writer, predict_examples, result):
            for predict_line, prediction in zip(predict_examples, result):
                idx = 0
                line = ""
                line_token = str(predict_line.text).split(' ')
                len_seq = len(line_token)
                print("predict_line:{}".format(line_token))
                print("prediction:{}".format(prediction))
                for index, id in enumerate(prediction[1:]):
                    if idx > len_seq:
                        break
                    if id == 0:
                        continue
